/* @ds-bundle: {"format":3,"namespace":"ArkodeDesignSystem_043916","components":[],"sourceHashes":{"kit/acompanamiento.js":"cef81b787c58","kit/deck-stage.js":"208980974db4","kit/image-slot.js":"9309434cb09c","kit/mosaic-canvas.js":"9e4aaf0b5b29","kit/mosaic-reveal.js":"edc1a6c33425","kit/motion.js":"87e9f314da7d","kit/pixel-option3.js":"88e55bbd059f"},"inlinedExternals":[],"unexposedExports":[]} */

(() => {

const __ds_ns = (window.ArkodeDesignSystem_043916 = window.ArkodeDesignSystem_043916 || {});

const __ds_scope = {};

(__ds_ns.__errors = __ds_ns.__errors || []);

// kit/acompanamiento.js
try { (() => {
/* Arkode · Acompañamiento Post Go-Live — interactions
   Scroll progress, reveals, count-up, scenario→model, model toggle,
   recommender quiz, tier selector, hours visualizer, compare modal.
...
(function () {
  ...
  function scrollUI() {
    ...
    var smTiles = $('#smTiles'),
      smLab = $('#smLab');
    // build mobile tiles to match
    if (smTiles && !smTiles.childElementCount) {
      links.forEach(function () {
        smTiles.appendChild(document.createElement('i'));
      });
    }
    var mTiles = smTiles ? $$('i', smTiles) : [];
    var labels = links.map(function (a) {
      var l = $('.lab', a);
      return l ? l.innerHTML : '';
    });
    function on() {
      var h = document.documentElement,
        sc = h.scrollTop / (h.scrollHeight - h.clientHeight || 1);
      if (bar) bar.style.width = sc * 100 + '%';
      if (tb) tb.classList.toggle('scrolled', h.scrollTop > 16);
      var y = h.scrollTop + h.clientHeight * 0.34,
        best = 0;
      ...
      links.forEach(function (a, i) {
        ...
        a.classList.toggle('done', i < best);
      });
      mTiles.forEach(function (t, i) {
        t.classList.toggle('on', i === best);
        t.classList.toggle('done', i < best);
      });
      if (smLab) smLab.innerHTML = labels[best] || '';
      // spine fill height: from top to active tile centre
      if (fill && spine && links[best]) {
        var sr = spine.getBoundingClientRect(),
          ar = links[best].getBoundingClientRect();
        var tileC = ar.top - sr.top + ar.height / 2;
        fill.style.height = Math.max(0, tileC - 20) + 'px';
      }
    }
  ...
  function reveals() {
    ...
    var io = new IntersectionObserver(function (es) {
      es.forEach(function (e) {
        if (e.isIntersecting) {
          ...
          if (e.target.hasAttribute('data-bars')) growBars(e.target);
        }
      });
    }, {
      threshold: 0.12,
    ...
    setTimeout(function () {
      $$('.reveal:not(.in)').forEach(function (e) {
        e.classList.add('in');
      });
    }, 2800);
  }
  ...
  function growBars(wrap) {
    $$('[data-h]', wrap).forEach(function (b, i) {
  ...
  function scenarios() {
    ...
    cards.forEach(function (c) {
      c.addEventListener('click', function () {
        ...
        c.classList.add('on');
        var rec = c.getAttribute('data-rec');
        var out = $('#scnOut');
        if (out) {
          out.querySelectorAll('[data-recname]').forEach(function (n) {
            n.classList.toggle('show', n.getAttribute('data-recname') === rec);
          });
  ...
  function modelToggle() {
    var wrap = $('#modelToggle');
    if (!wrap) return;
    var opts = $$('.pt-opt', wrap),
      ind = $('.pt-ind', wrap);
    var panels = $$('[data-model]');
    function set(i) {
      ...
      panels.forEach(function (p) {
        p.classList.toggle('show', p.getAttribute('data-model') === String(i));
      });
    }
    ...
    qs.forEach(function (q) {
      ...
      $$('.q-opt', q).forEach(function (opt) {
        opt.addEventListener('click', function () {
          $$('.q-opt', q).forEach(function (o) {
            o.classList.remove('on');
          });
          opt.classList.add('on');
          answers[key] = opt.getAttribute('data-val');
          var done = qs.every(function (qq) {
            return answers[qq.getAttribute('data-q')];
          });
          if (calcBtn) calcBtn.disabled = !done;
        });
      });
    });
    if (calcBtn) {
      calcBtn.addEventListener('click', function () {
        // scoring: model (sub vs hours) + tier
        var sub = 0,
          hrs = 0,
          tier = 0;
        if (answers.modelo === 'recurrente') sub += 2;
        if (answers.modelo === 'bolsa') hrs += 2;
        if (answers.continuidad === 'si') sub += 2;
        if (answers.continuidad === 'no') hrs += 1;
        if (answers.proyectos === 'varios') {
          tier += 2;
        }
        if (answers.proyectos === 'uno') {
          hrs += 1;
        }
        if (answers.ai === 'estrategico') {
          sub += 1;
          tier += 2;
        }
        if (answers.ai === 'talvez') {
          tier += 1;
        }
        var model = hrs > sub ? 'horas' : 'suscripcion';
        var plan;
        if (model === 'horas') {
          plan = {
            k: 'Paquete de Horas',
            t: 'Bolsa flexible',
            d: 'Proyectos puntuales, sin compromiso recurrente. Pagas por lo que usas.',
            tag: 'HORAS'
          };
        } else if (tier >= 4) {
          plan = {
            k: 'Suscripción · Centurion',
            t: 'Ventaja competitiva',
            d: 'Equipo embebido + agentes de AI estratégicos. El sistema se vuelve tu ventaja.',
            tag: 'CENTURION'
          };
        } else if (tier >= 2) {
          plan = {
            k: 'Suscripción · Platinum',
            t: 'Equipo embebido',
            d: 'Soporte continuo, desarrollo y un roadmap revisado contigo cada mes.',
            tag: 'PLATINUM'
          };
        } else {
          plan = {
            k: 'Suscripción · Gold',
            t: 'Continuidad tranquila',
            d: 'Que nada se rompa: soporte, mantenimiento y monitoreo proactivo.',
            tag: 'GOLD'
          };
        }
        var out = $('#quizOut');
        if (out) {
          $('#quizOutTag').textContent = plan.tag;
          $('#quizOutName').textContent = plan.k;
          $('#quizOutT').textContent = plan.t;
          $('#quizOutD').textContent = plan.d;
          out.classList.add('show');
          out.scrollIntoView({
            block: 'nearest'
          });
  ...
  /* ---------- tier selector (Gold/Platinum/Centurion) ---------- */
  function tiers() {
    var wrap = $('#tiers');
    if (!wrap) return;
    var tabs = $$('.tier-tab', wrap);
    var panels = $$('.tier-panel');
    tabs.forEach(function (t, i) {
      t.addEventListener('click', function () {
        ...
        t.classList.add('on');
        var key = t.getAttribute('data-tier');
        panels.forEach(function (p) {
          p.classList.toggle('show', p.getAttribute('data-tier') === key);
        });
      });
    });
  }

  /* ---------- hours visualizer (hover segments) ---------- */
  function hours() {
    var wrap = $('#hoursViz');
    if (!wrap) return;
    var segs = $$('.hv-seg', wrap),
      rows = $$('.hv-row', wrap);
    function focus(key) {
      segs.forEach(function (s) {
        s.classList.toggle('dim', key && s.getAttribute('data-cat') !== key);
      });
      rows.forEach(function (r) {
        r.classList.toggle('on', key && r.getAttribute('data-cat') === key);
      });
    }
    segs.concat(rows).forEach(function (el) {
      el.addEventListener('mouseenter', function () {
        focus(el.getAttribute('data-cat'));
      });
  ...
  function modal() {
    var m = $('#compareModal');
    if (!m) return;
    $$('[data-open-compare]').forEach(function (b) {
  ...
  /* ---------- logo sting (pixel mark assembles) ---------- */
  var MARK = ['.nnnnncc', 'nn...ncc', 'n.....n.', 'n.....n.', 'n.....n.', 'nn...nn.', '.nnnnn..'];
  function logoSting() {
    var reduce = window.matchMedia('(prefers-reduced-motion:reduce)').matches;
    $$('[data-sting]').forEach(function (host) {
      var unit = parseInt(host.getAttribute('data-unit'), 10) || 13;
      var base = host.hasAttribute('data-on-dark') ? '#F4ECDE' : '#001C43';
      host.style.cssText = 'position:relative;display:inline-block;width:' + 8 * unit + 'px;height:' + 7 * unit + 'px';
      host.innerHTML = '';
      var cells = [];
      for (var y = 0; y < 7; y++) {
        for (var x = 0; x < MARK[y].length; x++) {
          ...
          d.style.cssText = 'position:absolute;left:' + x * unit + 'px;top:' + y * unit + 'px;width:' + unit + 'px;height:' + unit + 'px;background:' + (ch === 'c' ? '#FF6C5D' : base) + ';opacity:0;transform:scale(0.2)';
          host.appendChild(d);
          cells.push(d);
        }
      }
      function play() {
        ...
        cells.forEach(function (c, i) {
          c.style.transition = 'opacity .4s var(--ease), transform .4s cubic-bezier(.34,1.56,.64,1)';
          c.style.transitionDelay = (reduce ? 0 : i * 45) + 'ms';
          c.style.opacity = '1';
  ...
  function tagStagger() {
    ...
    var io = new IntersectionObserver(function (es) {
      es.forEach(function (e) {
        if (e.isIntersecting) {
          [].slice.call(e.target.children).forEach(function (t, i) {
            setTimeout(function () {
              t.classList.add('tin');
            }, i * 60);
          });
  ...
  /* ---------- tier capacity meters (pixel tiles ∝ hours) ---------- */
  function capacityMeters() {
    var reduce = window.matchMedia('(prefers-reduced-motion:reduce)').matches;
    var TOTAL = 20,
      MAXH = 80;
    $$('.lad-col').forEach(function (col) {
      var cap = parseFloat(col.getAttribute('data-cap')) || 0;
      var lit = Math.round(cap / MAXH * TOTAL);
      var grid = $('.cap', col);
      if (!grid) return;
      grid.innerHTML = '';
      var cells = [];
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/acompanamiento.js", error: String((e && e.message) || e) }); }
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/deck-stage.js", error: String((e && e.message) || e) }); }
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/image-slot.js", error: String((e && e.message) || e) }); }

// kit/mosaic-canvas.js
try { (() => {
...
(function () {
  ...
  function mount(canvas) {
    if (canvas.__mzMounted) return;
    canvas.__mzMounted = true;
    ...
    function init() {
      cells = [];
      for (var i = 0; i < cols * rows; i++) cells.push({
        from: pick(),
        to: pick(),
        t: 1
      });
  ...
  function hydrate(root) {
    (root || document).querySelectorAll('canvas[data-mz]').forEach(mount);
  }
  window.MosaicCanvas = {
    hydrate: hydrate,
    mount: mount
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/mosaic-canvas.js", error: String((e && e.message) || e) }); }

// kit/mosaic-reveal.js
try { (() => {
/* Arkode Mosaic — shared reveal-on-scroll observer (fail-safe) */
(function () {
  try {
    if (!('IntersectionObserver' in window)) {
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/mosaic-reveal.js", error: String((e && e.message) || e) }); }

// kit/motion.js
try { (() => {
/* Arkode · Mosaic — Motion & Scroll library
   tag-stagger · hero-dissolve · sticky scrollytelling stage.
   (grain + marquee are pure CSS.) Vanilla, defensive, reduced-motion aware. */
(function () {
  'use strict';
  ...
  /* ---------- capability-tag stagger ---------- */
  function tagStagger() {
    if (!('IntersectionObserver' in window)) {
      $$('.tag-row').forEach(function (r) {
        $$('.tg', r).forEach(function (t) {
          t.classList.add('in');
        });
      });
      return;
  ...
  /* ---------- hero dissolve (scroll-linked parallax split) ---------- */
  function heroDissolve() {
    ...
    function frame() {
      ...
      // progress 0 (stage entering bottom) → 1 (stage top reaches ~top)
      var p = clamp((vh - r.top) / (vh * 0.9), 0, 1);
      // mosaic slides out left + fades; text floats toward center + up
      if (mz) {
        mz.style.transform = 'translateX(' + -p * 60 + '%)';
        mz.style.opacity = (1 - p * 0.9).toFixed(3);
      }
      if (tx) {
        tx.style.transform = 'translateX(' + p * -6 + '%) translateY(' + -p * 8 + 'px)';
      }
      raf = requestAnimationFrame(frame);
    }
  ...
  function scrolly() {
    var root = $('#scrolly');
    if (!root) return;
    var steps = $$('.scy-step', root);
    var panels = $$('.scy-panel', root);
    var dots = $$('.scy-dot', root);
    ...
    if ('IntersectionObserver' in window) {
      var io = new IntersectionObserver(function (es) {
        es.forEach(function (e) {
          if (e.isIntersecting) {
            setActive(+e.target.getAttribute('data-i'));
          }
        });
      }, {
        threshold: 0.6,
  ...
  /* ---------- line-draw chart ---------- */
  function lineDraw() {
    $$('[data-linedraw]').forEach(function (svg) {
      var paths = $$('path.draw, polyline.draw', svg);
          ...
          es.forEach(function (e) {
            if (e.isIntersecting) {
              paths.forEach(function (p, i) {
                p.style.transition = 'stroke-dashoffset 1.2s ' + 'cubic-bezier(0.22,0.68,0,1)';
                p.style.transitionDelay = i * 120 + 'ms';
                p.style.strokeDashoffset = reduce ? '0' : '0';
              });
              io.unobserve(e.target);
            }
          });
  ...
  /* ---------- donut sweep ---------- */
  function donutSweep() {
    $$('[data-donut]').forEach(function (svg) {
      var ring = $('.ring', svg);
      if (!ring) return;
      var pct = parseFloat(svg.getAttribute('data-donut')) || 0;
      var r = ring.r.baseVal.value,
        c = 2 * Math.PI * r;
      ring.style.strokeDasharray = c;
      ring.style.strokeDashoffset = c;
      ring.style.transition = 'none';
      var lbl = $('.donut-val', svg.parentNode);
      if ('IntersectionObserver' in window) {
        var io = new IntersectionObserver(function (es) {
          es.forEach(function (e) {
            if (e.isIntersecting) {
              ring.style.transition = 'stroke-dashoffset 1.3s cubic-bezier(0.22,0.68,0,1)';
              ring.style.strokeDashoffset = c * (1 - pct / 100);
              if (lbl) {
                var t0 = null;
                (function step(ts) {
                  if (!t0) t0 = ts;
                  var k = Math.min(1, (ts - t0) / 1300);
                  lbl.textContent = Math.round(pct * (1 - Math.pow(1 - k, 3))) + '%';
                  if (k < 1) requestAnimationFrame(step);
                })(performance.now());
              }
              io.unobserve(e.target);
  ...
  /* ---------- data-driven mosaic ---------- */
  function dataMosaic() {
    $$('[data-mosaic-pct]').forEach(function (grid) {
      var pct = parseFloat(grid.getAttribute('data-mosaic-pct')) || 0;
      var cols = parseInt(grid.getAttribute('data-cols'), 10) || 10;
      var rows = parseInt(grid.getAttribute('data-rows'), 10) || 5;
      var total = cols * rows,
        lit = Math.round(total * pct / 100);
      ...
      // shuffle order of lighting for organic fill, but light the first `lit`
      var order = cells.map(function (_, i) {
        return i;
      });
      function run() {
        cells.forEach(function (c) {
  ...
  /* ---------- logo sting (pixel mark assembles tile-by-tile) ---------- */
  var MARK = ['.nnnnncc', 'nn...ncc', 'n.....n.', 'n.....n.', 'n.....n.', 'nn...nn.', '.nnnnn..'];
  function buildSting(host) {
    var unit = parseInt(host.getAttribute('data-unit'), 10) || 14;
    var onDark = host.hasAttribute('data-on-dark');
    var base = onDark ? '#F4ECDE' : '#001C43';
    var cols = 8,
      rows = 7;
    host.style.position = 'relative';
    host.style.width = cols * unit + 'px';
    host.style.height = rows * unit + 'px';
    host.innerHTML = '';
    var cells = [];
  ...
  function logoSting() {
    $$('[data-sting]').forEach(function (host) {
      ...
      function play() {
        ...
        void host.offsetWidth;
        cells.forEach(function (c, i) {
          c.style.transition = 'opacity .4s var(--ease,cubic-bezier(.22,.68,0,1)), transform .4s cubic-bezier(.34,1.56,.64,1)';
          c.style.transitionDelay = (reduce ? 0 : i * 45) + 'ms';
          c.style.opacity = '1';
          c.style.transform = 'scale(1)';
        });
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/motion.js", error: String((e && e.message) || e) }); }

// kit/pixel-option3.js
try { (() => {
/* Arkode · Option 3 "Mosaic" — pixel-art sprite engine
   Renders crisp pixel sprites via a single box-shadow string (one DOM node per sprite).
   Usage in HTML:  <span data-px="arrowR" data-unit="4"></span>
                   <span data-px="bolt" data-unit="5" data-tint="#fff"></span>
*/
(function () {
  var PAL = {
    n: '#001C43',
  ...
  // Drawn on a consistent 7x7 grid (icons) with 1-unit stroke weight, symmetric,
  // legible silhouette at small sizes. Arrows = shaft + converging head.
  var SPRITES = {
    // → clean arrow: full shaft, head converging to the tip
    arrowR: ['.......', '....n..', '.....n.', 'nnnnnnn', '.....n.', '....n..', '.......'],
    // ↓ clean down arrow: shaft + solid triangular head
    arrowDownFull: ['...n...', '...n...', '...n...', 'nnnnnnn', '.nnnnn.', '..nnn..', '...n...'],
    // ⚡ lightning: extended single-pixel tips + filled kink — reads as a true bolt
    ...
    // ⚙ gear: 4 teeth, solid body, centred hole
    ...
    // 🌐 globe: circle with meridian + equator
    globe: ['..sss..', '.s.s.s.', 's..s..s', 'sssssss', 's..s..s', '.s.s.s.', '..sss..'],
    // 🤖 robot head with antenna, eyes and mouth
    bot: ['...n...', '.nnnnn.', 'nnnnnnn', 'n.n.n.n', 'nnnnnnn', 'n.nnn.n', '.nnnnn.'],
    // 💼 briefcase: handle, body, centre latch
    brief: ['..ooo..', '..o.o..', 'ooooooo', 'o.....o', 'o..o..o', 'o.....o', 'ooooooo'],
    // + plus
    plus: ['...c...', '...c...', '...c...', 'ccccccc', '...c...', '...c...', '...c...'],
    // ✓ checkmark: short stroke rising into a long stroke
    check: ['......g', '.....gg', '....gg.', '...gg..', 'g.gg...', 'ggg....', '.g.....'],
    // 🔌 plug: two prongs, body, cord
    plug: ['.n...n.', '.n...n.', 'nnnnnnn', 'nnnnnnn', '.nnnnn.', '...n...', '...n...'],
    // ▦ four-square mosaic glyph
    squares: ['cc.oo', 'cc.oo', '.....', 'nn.cc', 'nn.cc'],
    // Arkode mark: ring + coral node biting the top-right
    mark: ['.nnnnncc', 'nn...ncc', 'n.....n.', 'n.....n.', 'n.....n.', 'nn...nn.', '.nnnnn..'],
    // 📦 package: lidded box with vertical seam
    cube: ['.ooooo.', 'oo.o.oo', 'ooooooo', 'o..o..o', 'o..o..o', 'o..o..o', 'ooooooo'],
    // ◎ target: ring with a coral centre
    target: ['..nnn..', '.n...n.', 'n.....n', 'n..c..n', 'n.....n', '.n...n.', '..nnn..']
  };

  /* ---- Pixel weights ----------------------------------------------------
     Two options only, per Arkode usage rules:
       inset  (86% fill) — small sizes & dense UI; stays legible at unit ≤ 3
       fine   (70% fill) — the premium default for feature/standalone icons
  ...
  // Build a sprite node: each painted cell is an <i> sized to the weight's
  // fill ratio, centred in its cell, with a soft corner radius.
...
})(); } catch (e) { __ds_ns.__errors.push({ path: "kit/pixel-option3.js", error: String((e && e.message) || e) }); }
